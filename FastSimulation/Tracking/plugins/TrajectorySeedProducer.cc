...
{  
  ...
  recHitRefToken = consumes<RecHitCombinations>(hitProducer);
...
{        
  ...
  edm::Handle<RecHitCombinations> theRecHitCombinations;
  ...
  if(theRecHitCombinations->size() == 0)
    {
  ...
  for (unsigned int c=0; c < theRecHitCombinations->size(); ++c ){	    
    ...
    const STMatchedRecHitRefCollection & theRecHitCombination = theRecHitCombinations->at(c);
