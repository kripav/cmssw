...
//   }
  ...
  if(det.subdetId() > 2) {
    ...
    SiTrackerGSMatchedRecHit2D *rV= new SiTrackerGSMatchedRecHit2D(position, error, *gluedDet,
								   monoRH->Id(),
								   monoRH->simtrackId1(),								  
								   monoRH->simtrackId2(), monoRH->eeId(), monoRH->cluster(), 
					            ...
					            LocalVector& ldir) const
{
  ...
  SiTrackerGSRecHit2D *otherRH = new SiTrackerGSRecHit2D(LocalPoint(-10000,-10000,-10000), LocalError(0,0,0),*otherDet, 0,0,0,0,0,monoRH->cluster(),0,0);
